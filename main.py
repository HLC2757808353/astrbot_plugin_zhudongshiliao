...
class MyPlugin(Star):
    ...
    async def _inject_cross_session_context(
        ...
        target_umo = f"{platform_id}:FriendMessage:{target_user_id}"
        ...
        try:
            ...
            context_content = f"[跨会话通知] {sender_name}在{source_type}「{source_name}」让你给当前用户发了消息：「{message_content}」。以下是来源对话的最近内容，帮助你理解前因后果：\n{recent_summary}"
            ...
            logger.info(f"已注入跨会话上下文到 {target_umo}（来源: {source_name}，含{len(context_parts)}条上下文）")
        except Exception as e:
    ...
    async def _inject_source_confirmation(
        ...
        try:
            conv_id = await self.context.conversation_manager.get_curr_conversation_id(source_umo)
            if not conv_id:
                return
            
            conv = await self.context.conversation_manager.get_conversation(source_umo, conv_id)
            if not conv:
                return
            ...
            logger.info(f"已向来源会话 {source_umo} 注入发送确认")
        except Exception as e:
    ...
    async def _inject_cross_session_context_to_group(
        ...
        target_umo = f"{platform_id}:GroupMessage:{target_group_id}"
        ...
        try:
            ...
            context_content = f"[跨会话通知] {sender_name}在{source_type}「{source_name}」让你给当前群发了消息：「{message_content}」。以下是来源对话的最近内容，帮助你理解前因后果：\n{recent_summary}"
            ...
            logger.info(f"已注入跨会话上下文到群 {target_umo}（来源: {source_name}，含{len(context_parts)}条上下文）")
        except Exception as e:
    ...
    def _is_error_message(self, text: str) -> bool:
        for pattern in self.STRICT_ERROR_PATTERNS:
    ...
    async def _call_llm_for_proactive(self, umo: str) -> str | None:
        if not umo:
            logger.warning("会话UMO为空，无法生成主动回复。请先与管理员私聊一次。")
            return None
    ...
    async def _call_llm_for_dialogue_continuation(self, umo: str, current_round: int, max_rounds: int) -> str | None:
        if not umo:
            logger.warning("会话UMO为空，无法生成对话延续")
            return None
    ...
    def _is_in_active_time_window(self):
        ...
        logger.debug(f"时间窗口检查: 本地{current_hour}点(UTC+{tz_offset}), 窗口{start_hour}-{end_hour}, 结果={'活跃' if result else '不活跃'}")
        return result
    ...
    async def _execute_proactive_reply(self):
        umo = self._admin_umo
        if not umo:
            logger.warning("尚未有管理员私聊过，请先私聊绑定管理员")
            return
        ...
        if self._last_user_activity_time is None and self.context.conversation_manager:
            try:
                conv_id = await self.context.conversation_manager.get_curr_conversation_id(umo)
                if conv_id:
                    conv = await self.context.conversation_manager.get_conversation(umo, conv_id)
                    if conv and conv.history:
                        ...
                        for msg in reversed(history):
                            if msg.get("role") == "user" and msg.get("created_at"):
                                self._last_user_activity_time = float(msg["created_at"])
                                logger.info(f"已从历史恢复最后活动时间: {msg['created_at']}")
                                break
                        if self._last_user_activity_time is None:
                            self._last_user_activity_time = time.time() - 3600
                            logger.info("历史中无用户消息时间戳，使用默认值")
                    else:
                        self._last_user_activity_time = time.time() - 3600
                else:
                    self._last_user_activity_time = time.time() - 3600
            except Exception as e:
                logger.warning(f"恢复历史活动时间失败: {e}，使用默认值")
                self._last_user_activity_time = time.time() - 3600
        
        logger.info(f"正在调用 LLM 生成主动回复内容（UMO: {umo}）...")
        ...
        while True:
            try:
                ...
                if self._consecutive_failures >= self._max_consecutive_failures:
                    logger.warning(f"连续失败 {self._consecutive_failures} 次，暂停主动回复10分钟")
                ...
                logger.warning(f"异常后等待 {backoff_time} 秒（连续失败 {self._consecutive_failures} 次）")
    ...
    async def terminate(self):
        ...
        self._config_cache = None
        logger.info("主动私聊插件已停止，所有后台任务和缓存已清理")
